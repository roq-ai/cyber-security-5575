...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createService() {
    ...
    const body = { ...req.body };
    if (body?.company_service?.length > 0) {
      const create_company_service = body.company_service;
      body.company_service = {
        create: create_company_service,
      };
    } else {
      delete body.company_service;
    }
