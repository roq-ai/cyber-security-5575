...
import { getCompanyServices, deleteCompanyServiceById } from 'apiSdk/company-services';
import { CompanyServiceInterface } from 'interfaces/company-service';

type ColumnType = ColumnDef<CompanyServiceInterface, unknown>;

interface CompanyServiceListPageProps {
...
export function CompanyServiceListPage(props: CompanyServiceListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<CompanyServiceInterface>>(
    () => `/company-services?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await deleteCompanyServiceById(id);
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: CompanyServiceInterface) => {
    if (hasAccess('company_service', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/company-services/view/${row.id}`);
    }
  };
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/company-services/view/${record.id}`} passHref legacyBehavior>
            <Button
          ...
          {hasAccess('company_service', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/company-services/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('company_service', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="company_service" />
        </Flex>

        {hasAccess('company_service', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/company-services/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
